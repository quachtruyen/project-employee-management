...
def get_department_by_id(request, dep_id):
    employee_list = employee_model.objects.filter(department_id = dep_id).order_by('emp_id')
    department = department_model.objects.get(department_id = dep_id)
